...
def get_kafka_producer():
    try:
        return Producer({'bootstrap.servers': 'kafka1:9092,kafka2:9092'})
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Kafka producer error: {str(e)}")
...
def send_to_kafka(topic, message):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to send message to Kafka: {str(e)}")
...
def create_user(user: UserCreate, current_user: User = Depends(get_current_client), db: Session = Depends(get_db)):
    try:
        ...
        send_to_kafka(KAFKA_TOPIC, user_message)
